...
and e.g. Sub-Zero Containment's frozen Ridley stood tall until mid-game).
`pipeline/tileOverrides.<game>.json` lists them per area as
`{ x, y, image, sx, sy }` in **tile coords**; `image` is `pipeline/`-relative,
...
Draw data is all-or-nothing, and `AreaCell`'s union type enforces it: `if (!c.k) return;` narrows `c.w` to a number. Whether a cell is _served_ as a target is not this list's job: **the map must draw something there** (`drawnCells` in `src/data.ts`) and difficulty must allow it (`EXCLUDED_RATING` = 6, never served). There is no "secret"/excluded cell flag; don't add one.

`drawnCells` is draw data **or** a connector running over the cell — transit is drawn too, and a junction like ZM Norfair's `14,13` (Speed Transporter B) is a real place you visit and remember. It gates both ends of a round, and the two ends must stay in step: `pickTargets` won't serve an undrawn cell, and `GuessMap`'s hit-test (`guessable`) won't let a click land on one — nothing is drawn there, so a click could only ever be a mis-click, and a target nobody can find is an unwinnable round. A connector cell with no cell behind it (Crateria's arrow-only runs) doesn't count: no screen, nothing to guess. X-Ray still paints every cell, drawn or not; that's the whole point of keeping them. `cellPool` deliberately keeps counting undrawn cells so old `?seed=` links keep resolving to the same rooms.
...
A connector is only _how a run gets drawn_; the cells under it are ordinary `area.cells` entries that simply carry no draw data (no `k`), so the map draws the connector rather than a room box. They're real tiles, so the X-Ray overlay (`showTiles`) paints the actual elevator shaft / tube there — and since the connector draws them, they _are_ clickable and servable, so rate them **difficulty 6** unless the cell is somewhere a player would actually recognize (SM's Toilet Bowl, ZM's Speed Transporter junction — those are rated normally and served on purpose). Connectors whose source art is only an exit arrow + caption (all of Crateria's and Wrecked Ship's) have no tile at all — they fall under `slice_maps.py`'s fill threshold, so no cell exists and nothing is drawn but the connector.
