...
Draw data is all-or-nothing, and `AreaCell`'s union type enforces it: `if (!c.k) return;` narrows `c.w` to a number. Whether a cell is _served_ as a target is not this list's job: it must be **charted** (`isCharted` in `src/data.ts` — draw data present) and difficulty must allow it (`EXCLUDED_RATING` = 6, never served). There is no "secret"/excluded cell flag; don't add one.

Charted-ness gates both ends of a round, and the two ends must stay in step: `pickTargets` won't serve an uncharted cell, and `GuessMap`'s hit-test (`guessable`) won't let a click land on one — nothing is drawn there, so a click could only ever be a mis-click, and a target nobody can click is an unwinnable round. X-Ray still paints every cell, charted or not; that's the whole point of keeping them. `cellPool` deliberately keeps counting uncharted cells so old `?seed=` links keep resolving to the same rooms.
...
A connector is only _how a run gets drawn_; the cells under it are ordinary `area.cells` entries that simply carry no draw data (no `k`), so the map draws the connector rather than a room box. They're real tiles, so the X-Ray overlay (`showTiles`) paints the actual elevator shaft / tube there — but being uncharted they can't be clicked or served, so a difficulty rating on them is belt-and-braces (**6** by convention) rather than the thing keeping them out of play. Connectors whose source art is only an exit arrow + caption (all of Crateria's and Wrecked Ship's) have no tile at all — they fall under `slice_maps.py`'s fill threshold, so no cell exists and nothing is drawn but the connector.
